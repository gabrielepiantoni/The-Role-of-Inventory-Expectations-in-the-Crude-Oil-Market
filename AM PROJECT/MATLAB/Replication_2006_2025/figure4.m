...
time_plot = time(p+1 : p+length(yhat1));
...
axis([time_plot(1) time_plot(end) -1.5 0.5])
grid on
...
% Save the key vectors to a .mat file
% 'yhat1_rep', 'yhat2_rep', 'yhat3_rep' = Replication (3-var) results
yhat1_rep = yhat1;
yhat2_rep = yhat2;
yhat3_rep = yhat3;
time_rep  = time_plot;

outdir = fullfile('..','Extension_Level');

if ~exist(outdir,'dir')
    mkdir(outdir);
end

save(fullfile(outdir,'results_replication_3var_2006_2025_fig4.mat'), ...
     'yhat1_rep','yhat2_rep','yhat3_rep','time_rep');
